...
echo "python3 AGENTS/skills/slide_preparation/scripts/run.py $ROOT $TASK_ID" >> "$CMD_LOG"
if command -v python3 >/dev/null 2>&1; then
  python3 "$ROOT/AGENTS/skills/slide_preparation/scripts/run.py" "$ROOT" "$TASK_ID" >> "$STDOUT_LOG" 2>> "$STDERR_LOG"
elif command -v python >/dev/null 2>&1; then
  python "$ROOT/AGENTS/skills/slide_preparation/scripts/run.py" "$ROOT" "$TASK_ID" >> "$STDOUT_LOG" 2>> "$STDERR_LOG"
else
