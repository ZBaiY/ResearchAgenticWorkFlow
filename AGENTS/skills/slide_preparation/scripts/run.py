...
from approval import clarify_text, confirm
...
def main() -> int:
    ...
    if not req["talk"].get("duration_min"):
        ans = clarify_text("Missing talk.duration_min. Enter duration in minutes: ", "20")
        if ans:
    ...
    if not req["talk"].get("audience"):
        ans = clarify_text("Missing talk.audience. Enter expert/mixed/adjacent/general: ", "mixed")
        req["talk"]["audience"] = ans or "mixed"
        questions_asked.append("audience")

    if not req["talk"].get("venue"):
        ans = clarify_text("Missing talk.venue. Enter conference/seminar/group meeting: ", "seminar")
        req["talk"]["venue"] = ans or "seminar"
        questions_asked.append("venue")

    if not req["talk"].get("emphasis") and not req["talk"].get("goal"):
        ans = clarify_text("Missing emphasis/goal. Enter one short phrase for emphasis or goal: ", "")
        if ans:
