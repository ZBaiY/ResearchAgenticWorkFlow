...
if [[ ! -d "$META_DIR" || -z "$(find "$META_DIR" -maxdepth 1 -name '*.json' -print -quit 2>/dev/null)" ]]; then
  echo "Resources metadata not found. Run: bash AGENTS/skills/jhep_writer/fetch_resources.sh" >> "$STDOUT_LOG"
fi
...
% Run: bash AGENTS/skills/jhep_writer/fetch_resources.sh
...

{
  ...
  if [[ -d "$META_DIR" ]]; then
    ...
    if [[ "$found_any" == "false" ]]; then
      echo "- No metadata JSON found. Run: bash AGENTS/skills/jhep_writer/fetch_resources.sh"
    fi
  else
    echo "- No resources/meta directory found. Run: bash AGENTS/skills/jhep_writer/fetch_resources.sh"
  fi
