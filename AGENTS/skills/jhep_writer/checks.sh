...
if compgen -G "$VENDOR_JHEP/*jheppub*.sty" > /dev/null; then
  if [[ -f "$SHADOW_PAPER/main.tex" ]]; then
    if rg -q '\\usepackage\{jheppub\}' "$SHADOW_PAPER/main.tex" || rg -q 'jheppub\.sty not found|TEXINPUTS' "$SHADOW_PAPER/main.tex"; then
      JHEP_REF_STATUS="ok"
    else
      JHEP_REF_STATUS="fail"
    fi
  fi
else
  JHEP_REF_STATUS="skipped"
