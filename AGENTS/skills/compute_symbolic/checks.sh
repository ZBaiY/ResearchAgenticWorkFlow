...
if rg -q 'python3|python ' "$CMDS"; then
  echo "Forbidden python command found in symbolic skill logs"
  exit 1
fi
if ! rg -q 'wolframscript|math -script|backend unavailable' "$CMDS"; then
  echo "No symbolic backend command/marker found"
  exit 1
fi
...
echo "compute_symbolic checks: ok"
