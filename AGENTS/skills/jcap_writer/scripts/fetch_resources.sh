...
UA="CodexCLI-JcapWriter/1.0 (+https://jcap.sissa.it)"
...
extract_style_links() {
  ...
  grep -Eoi 'https?://[^"<> ]*(jcappub\.sty|template[^"<> ]*\.tex|\.zip)' "$html_file" | sort -u || true
}

FETCH_LOG=()

# 1) JCAP TeXclass help page
fetch_file \
  "https://jcap.sissa.it/jcap/help/JCAP_TeXclass.jsp" \
  "$RES_DIR/jcap_texclass_help.html" \
  "JCAP TeXclass help page HTML snapshot"

# 2) JCAP author manual PDF
fetch_file \
...
STYLE_LINKS="$(extract_style_links "$RES_DIR/jcap_texclass_help.html")"
if [[ -n "$STYLE_LINKS" ]]; then
  while IFS= read -r link; do
    [[ -n "$link" ]] || continue
    base="$(basename "$link")"
    fetch_file "$link" "$RES_DIR/$base" "Style/template resource discovered from JCAP TeXclass help page"
  done <<< "$STYLE_LINKS"
  rm -f "$STYLE_HELP_MD"
else
...
1. Open the TeXclass help page and locate links for \
   \`jcappub.sty\` and any official JCAP template source.
...
{
  echo "# JCAP Writer Offline Resources Status"
  echo
  ...
  echo
  echo "Rerun command:"
  echo "\`bash AGENTS/skills/jcap_writer/scripts/fetch_resources.sh\`"
} > "$STATUS_MD"
