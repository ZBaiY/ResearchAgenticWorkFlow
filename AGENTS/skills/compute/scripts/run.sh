...
if [[ -z "$backend" ]]; then
  ans="$(approval_clarify_text "Generic compute skill is deprecated. Choose backend: [1] numerical-python [2] symbolic-mathematica: " "1")"
...
bash "$ROOT/AGENTS/skills/$delegate_skill/scripts/run.sh" "$ROOT" "$TASK_ID"
