...
echo "python3 AGENTS/skills/literature_scout/scripts/run.py $ROOT $TASK_ID" >> "$CMD_LOG"
if command -v python3 >/dev/null 2>&1; then
  python3 "$ROOT/AGENTS/skills/literature_scout/scripts/run.py" "$ROOT" "$TASK_ID"
elif command -v python >/dev/null 2>&1; then
  python "$ROOT/AGENTS/skills/literature_scout/scripts/run.py" "$ROOT" "$TASK_ID"
else
